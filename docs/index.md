...
a locked row *unconditionally* shares one `case` label. `PostgreSQL`, `MySQL`,
`MariaDB`, and `Oracle` only skip above a version floor, so those arms consult
`supportsSkipLocked()` in a guard rather than being assumed:

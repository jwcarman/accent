...
/**
 * Db2. Tagged {@code heavy}: the image is licence-gated, slow to start, and publishes no {@code
 * linux/arm64} manifest, so on Apple Silicon it runs only under emulation. Treat this as
 * CI-verified rather than reliably reproducible on a workstation.
 *
 * <p>Db2's product name carries a host-architecture suffix — {@code DB2/LINUXX8664} here, {@code
 * DB2/NT64} elsewhere — so only a prefix match is safe. Its product version is {@code SQL120100}, a
 * build identifier from which nothing parses.
 */
@Tag("heavy")
@Testcontainers
class Db2IT {
